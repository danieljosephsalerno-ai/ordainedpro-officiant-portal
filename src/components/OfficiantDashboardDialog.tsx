...
interface OfficiantDashboardDialogProps {
  ...
  initialView?: "dashboard" | "ceremonies" | "profile" | "calendar" | "documents" | "settings"
}
...
export function OfficiantDashboardDialog({ open, onOpenChange, onSelectCouple, couples, onAddCeremony, initialView = "dashboard" }: OfficiantDashboardDialogProps) {
  // Subscription information
  const { subscription, isProfessional, isAspirant } = useSubscription()

  const [activeView, setActiveView] = useState<"dashboard" | "ceremonies" | "calendar" | "documents" | "profile" | "settings">(initialView)
  const [ceremonyFilter, setCeremonyFilter] = useState<"Active" | "Archived" | "All">("Active")
  const [searchQuery, setSearchQuery] = useState("")
  const [ceremonies, setCeremonies] = useState<Ceremony[]>([])
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date())
  const [showAddCeremonyDialog, setShowAddCeremonyDialog] = useState(false)
  const [documents, setDocuments] = useState<Array<{ id: string; name: string; size: string; type: string; updated: string }>>([])
    ...
    if (couples && couples.length > 0) {
      const transformedCeremonies: Ceremony[] = couples.map((couple) => {
        ...
        const weddingDate = couple.weddingDetails?.weddingDate
          ? new Date(couple.weddingDetails.weddingDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
          : 'TBD'
        ...
        const coupleColors = (couple as any).colors || { bride: "bg-pink-500", groom: "bg-blue-500" }
        ...
        return {
          ...
          time: couple.weddingDetails?.startTime
            ? new Date(`2000-01-01T${couple.weddingDetails.startTime}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })
            : 'TBD',
          ...
          guests: couple.weddingDetails?.expectedGuests ? parseInt(couple.weddingDetails.expectedGuests) : 0,
        }
      })
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      const defaultDocs = [
        { id: "1", name: "Wedding Ceremony Template", size: "245 KB", type: "PDF", updated: "2 days ago" },
        { id: "2", name: "Service Agreement", size: "156 KB", type: "PDF", updated: "1 week ago" },
        { id: "3", name: "Vows Examples", size: "389 KB", type: "PDF", updated: "3 weeks ago" },
      ]
  ...
  const filteredCeremonies = ceremonies.filter((ceremony) => {
    const matchesFilter = ceremonyFilter === "All" || ceremony.status === ceremonyFilter
    const matchesSearch =
      searchQuery === "" ||
  ...
  const ytdCompletedCeremonies = ceremonies.filter((c) => {
    if (!c.rawDate) return false
    const ceremonyDate = new Date(c.rawDate)
    return ceremonyDate.getFullYear() === currentYear && ceremonyDate < currentDate
  })
  ...
  const handleAddCeremony = () => {
    ...
    if (onAddCeremony) {
      // Transform to Couple format - matching Communication Portal exactly
      const newCouple: Partial<Couple> = {
    ...
    alert(`Ceremony "${newCeremony.ceremonyName}" for ${newCeremony.brideName} & ${newCeremony.groomName} has been saved successfully!\n\nThis couple has been added to your ceremony list.`)
  }

  const handleProfileUpdate = <K extends keyof OfficiantProfile>(field: K, value: OfficiantProfile[K]) => {
    setProfile(prev => {
  ...
  const handleHeadshotUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        handleProfileUpdate("headshot", reader.result as string)
      }
      ...
      Array.from(files).forEach(file => {
        ...
        reader.onloadend = () => {
          setProfile(prev => {
            const updated = {
              ...prev,
              photoGallery: [...prev.photoGallery, reader.result as string]
            }
            localStorage.setItem("officiantProfile", JSON.stringify(updated))
            return updated
          })
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 209715200) {
        alert("Video file must be less than 200MB")
        return
      }
      const reader = new FileReader()
      reader.onloadend = () => {
        handleProfileUpdate("videoUrl", reader.result as string)
      }
      reader.readAsDataURL(file)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] w-[1400px] h-[90vh] p-0 gap-0 overflow-hidden">
          ...
          <div className="w-64 bg-white border-r border-gray-200 p-6">
            ...
            <div className="mt-auto pt-8">
              <div className="bg-blue-50 rounded-lg p-4 text-sm">
                ...
                <p className="text-blue-700 text-xs mt-1">Contact support for assistance</p>
              </div>
            </div>
          </div>

          {/* Main Content */}
          <div className="flex-1 overflow-y-auto bg-gray-50" style={{ height: '100%', maxHeight: '90vh' }}>
            {/* Header */}
            <div className="bg-white border-b border-gray-200 px-8 py-6">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Welcome Back!</h1>
                  <p className="text-gray-600">Manage your ceremonies and profile</p>
                </div>
            ...
            {activeView === "dashboard" && (
              <div className="p-8">
                ...
                <p className="text-gray-600 mb-6">Overview of your ceremonies and activities</p>
                ...
                <div className="grid grid-cols-3 gap-6 mb-8">
                  <Card>
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-gray-600 text-sm">Total Ceremonies (YTD)</p>
                          <p className="text-3xl font-bold text-gray-900">{ytdCeremonies.length}</p>
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-gray-600 text-sm">Active Ceremonies</p>
                          <p className="text-3xl font-bold text-gray-900">{activeCeremonies.length}</p>
                          <p className="text-gray-500 text-xs mt-1">2 this week</p>
                        </div>
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-gray-600 text-sm">Completed (YTD)</p>
                          <p className="text-3xl font-bold text-gray-900">{ytdCompletedCeremonies.length}</p>
                          <p className="text-gray-500 text-xs mt-1">
                            {ytdCeremonies.length - ytdCompletedCeremonies.length} upcoming
                          </p>
                        </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-gray-900">Upcoming Ceremonies</h3>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              <div className="flex -space-x-2">
                                <Avatar className="border-2 border-white">
                                  <AvatarFallback className={ceremony.couple1Color}>{ceremony.couple1Initial}</AvatarFallback>
                                </Avatar>
                                <Avatar className="border-2 border-white">
                                  <AvatarFallback className={ceremony.couple2Color}>{ceremony.couple2Initial}</AvatarFallback>
                                </Avatar>
                              </div>
                              <div>
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Badge className={ceremony.status === "Active" ? "bg-green-500" : "bg-red-500"}>
                                    {ceremony.status === "Active" ? "Active Ceremony" : "Archived Ceremony"}
                                  </Badge>
                                </div>
                            ...
                            <div className="text-right">
                              <p className="text-sm text-blue-600 font-medium">
                                {ceremony.rawDate ? getDaysUntilCeremony(ceremony.rawDate) : 'Date TBD'}
                              </p>
            ...
            {activeView === "ceremonies" && (
              <div className="p-8">
                <h2 className="text-xl font-bold text-gray-900 mb-4">My Ceremonies</h2>
                <p className="text-gray-600 mb-6">Search and manage all your wedding ceremonies</p>

                {/* Filter Tabs */}
                <div className="flex items-center justify-between mb-6">
                  <div className="flex space-x-2">
                    <Button
                      variant={ceremonyFilter === "Active" ? "default" : "outline"}
                      className={ceremonyFilter === "Active" ? "bg-green-500 hover:bg-green-600" : ""}
                      onClick={() => setCeremonyFilter("Active")}
                    >
                      Active
                    </Button>
                    <Button
                      variant={ceremonyFilter === "Archived" ? "default" : "outline"}
            ...
            {activeView === "calendar" && (
              <div className="p-8">
                ...
                <p className="text-gray-600 mb-6">View and manage your ceremony schedule</p>
                          ...
                          >
                            <div className="flex items-center space-x-2 mb-2">
                              ...
                              <p className="text-sm font-medium text-purple-900">
                                {ceremony.date.split(",")[0].replace(/\d+/, (d) => d)}
                              </p>
                            </div>
            ...
            {activeView === "profile" && (
              <div className="p-8">
                <div className="mb-6">
                  <h2 className="text-xl font-bold text-gray-900">My Public Profile</h2>
                  <p className="text-gray-600">Manage your professional profile that couples will see when searching for officiants</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-1">
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-sm text-gray-600">Profile Preview</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        {/* Headshot */}
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-3 border-t">
                          <div className="text-center">
                            <p className="text-2xl font-bold text-gray-900">{profile.photoGallery.length}</p>
                            <p className="text-xs text-gray-500">Photos</p>
                          </div>
                          <div className="text-center">
                            <p className="text-2xl font-bold text-gray-900">{profile.videoUrl ? "1" : "0"}</p>
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>Your professional details and headshot</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            This name will appear on all invoices and official documents
                          </p>
                        </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="relative">
                              ...
                              <Input
                                ...
                                onChange={(e) => handleSocialMediaUpdate("linkedin", e.target.value)}
                            ...
                            <div className="relative">
                              ...
                              <Input
                                ...
                                onChange={(e) => handleSocialMediaUpdate("youtube", e.target.value)}
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => handleProfileUpdate("priceRange", {
                                ...profile.priceRange,
                        ...
                        <p className="text-sm text-gray-500">
                          This range will be displayed to couples searching for officiants in your area
                        </p>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>Tell couples about your experience and approach</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <Textarea
                          value={profile.bio}
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>Upload photos of ceremonies you've officiated</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div>
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>Upload a video introducing yourself (max 200MB)</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        {!profile.videoUrl ? (
                          <div>
                          ...
                          <div>
                            ...
                            <Button
                              variant="destructive"
                              onClick={() => handleProfileUpdate("videoUrl", "")}
                    ...
                    <div className="flex justify-end space-x-3">
                      ...
                      <Button
                        onClick={() => {
                          alert("Profile saved successfully! Your profile is now visible to couples searching for officiants.")
                        }}
            ...
            {activeView === "documents" && (
              <div className="p-8">
                ...
                <p className="text-gray-600 mb-6">Manage templates, contracts, and ceremony scripts</p>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-start space-x-3">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                              doc.type === "PDF" ? "bg-blue-100" : doc.type === "DOC" ? "bg-green-100" : "bg-purple-100"
                            }`}>
                              <FileText className={`w-5 h-5 ${
                                doc.type === "PDF" ? "text-blue-600" : doc.type === "DOC" ? "text-green-600" : "text-purple-600"
                              }`} />
                            </div>
                            <div>
                ...
                <Card className="border-2 border-dashed border-gray-300">
                  <CardContent className="pt-6">
                    <label htmlFor="file-upload-drop" className="cursor-pointer">
                      <div className="text-center py-12">
                        <Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                        <p className="font-medium text-gray-900 mb-1">Upload New Document</p>
            ...
            {activeView === "settings" && (
              <div className="p-8">
                ...
                <p className="text-gray-600 mb-6">Manage your subscription and account preferences</p>
                ...
                <div className="max-w-4xl">
                  <Card className="border-2 border-blue-200 shadow-lg mb-6">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <CardTitle className="flex items-center space-x-2 text-xl">
                            ...
                            <span>Current Plan: {subscription?.tier === 'professional' ? 'Professional' : 'Aspirant'}</span>
                          </CardTitle>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-gray-50 rounded-lg p-4">
                            <p className="text-sm text-gray-600 mb-1">Ceremonies Limit</p>
                          ...
                          <div className="bg-gray-50 rounded-lg p-4">
                            <p className="text-sm text-gray-600 mb-1">Scripts Limit</p>
                            <p className="text-2xl font-bold text-gray-900">
                      ...
                      {/* Feature Access */}
                      <div>
                        <h3 className="font-semibold text-gray-900 mb-3">Feature Access</h3>
                        <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                          {subscription && Object.entries(subscription.features).map(([feature, hasAccess]) => (
                      ...
                      {isProfessional && (
                        <div className="mt-6 pt-6 border-t">
                          ...
                          <p className="text-sm text-gray-500 mt-3">
                            Your subscription will renew on <strong>December 27, 2024</strong> for <strong>$29.00/month</strong>
                          </p>
                        </div>
                      )}
                    </CardContent>
                        ...
                        <Card className="border-2 border-blue-500 shadow-lg">
                          <CardHeader>
                            <div className="flex items-center justify-between mb-2">
                              <CardTitle className="text-lg">Aspirant</CardTitle>
                          ...
                          <CardContent>
                            <div className="space-y-3">
                              <div className="flex items-start space-x-2">
                                ...
                                <span className="text-sm text-gray-700">Up to 3 ceremonies</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Script builder access</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Basic scheduling</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                              ...
                              <div className="flex items-start space-x-2">
                                ...
                                <span className="text-sm text-gray-400">Messages & files</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <X className="w-5 h-5 text-gray-300 mt-0.5" />
                                <span className="text-sm text-gray-400">Invoices & payments</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <X className="w-5 h-5 text-gray-300 mt-0.5" />
                                <span className="text-sm text-gray-400">Marketplace access</span>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="border-2 border-purple-500 shadow-lg relative overflow-hidden">
                          ...
                          <CardContent>
                            <div className="space-y-3 mb-6">
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700"><strong>Unlimited</strong> ceremonies</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Advanced script builder</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Full messaging system</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Contract management</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Invoice & payment tracking</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Earnings dashboard</span>
                              </div>
                              <div className="flex items-start space-x-2">
                                <Check className="w-5 h-5 text-green-500 mt-0.5" />
                                <span className="text-sm text-gray-700">Marketplace script sales</span>
                              </div>
                            </div>
        ...
        <Dialog open={showPreview} onOpenChange={setShowPreview}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl">Public Profile Preview</DialogTitle>
              <DialogDescription>
                This is how your profile will appear to couples searching for officiants
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6 py-4">
                ...
                <Card className="hover:shadow-xl transition-all border-purple-100 max-w-md mx-auto">
                  <CardContent className="p-6">
                    ...
                    <div className="flex flex-col items-center mb-4">
                      ...
                      <h3 className="font-bold text-lg text-gray-900 text-center">{profile.fullName}</h3>
                    ...
                    <p className="text-sm text-gray-600 text-center line-clamp-3 mb-4">
                      {profile.bio || "Professional wedding officiant ready to make your ceremony special."}
                    </p>
              ...
              <div className="space-y-4 border-t pt-6">
                ...
                <div>
                  ...
                  {/* Social Media */}
                  {(profile.socialMedia.facebook || profile.socialMedia.instagram || profile.socialMedia.linkedin || profile.socialMedia.youtube) && (
