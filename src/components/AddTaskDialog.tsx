...
export interface AddTaskDialogProps {
  ...
  coupleId?: number | null  // ⚠️ CRITICAL: Pass couple_id for data isolation
  coupleName?: string       // Display name for context
}

export function AddTaskDialog({ onAddTask, isOpen, onOpenChange, coupleId, coupleName }: AddTaskDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ⚠️ CRITICAL: Use passed coupleId for data isolation
      if (!coupleId) {
        alert("⚠️ Please select a couple before creating a task.");
        setSaving(false);
        return;
      }

      const newTaskData = {
        couple_id: coupleId,  // ⚠️ Uses prop instead of hardcoded value
        user_id: userId,
        title: formData.task,
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {coupleName
              ? `Create a new task for ${coupleName} with priority level, due date, and notification settings`
