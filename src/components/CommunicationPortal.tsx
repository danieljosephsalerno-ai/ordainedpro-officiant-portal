...
Based on these selections, I'll create a beautiful ceremony for ${editCoupleInfo.brideName} & ${editCoupleInfo.groomName}. Let me focus on the remaining details to perfect your script:`
  ...
  const handleUploadScript = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const bride1FirstName = editCoupleInfo.brideName.split(' ')[0]
      const groom1FirstName = editCoupleInfo.groomName.split(' ')[0]

      // Common placeholder patterns to replace
      const placeholders = [
        // Full names
        { pattern: /\[Bride(?:'s)? (?:Full )?Name\]/gi, replacement: editCoupleInfo.brideName },
        { pattern: /\[Groom(?:'s)? (?:Full )?Name\]/gi, replacement: editCoupleInfo.groomName },
        { pattern: /\[Partner 1(?:'s)? Name\]/gi, replacement: editCoupleInfo.brideName },
        { pattern: /\[Partner 2(?:'s)? Name\]/gi, replacement: editCoupleInfo.groomName },
        { pattern: /\{Bride(?:'s)? Name\}/gi, replacement: editCoupleInfo.brideName },
        { pattern: /\{Groom(?:'s)? Name\}/gi, replacement: editCoupleInfo.groomName },
        ...
        { pattern: /BRIDE_NAME/g, replacement: editCoupleInfo.brideName },
        { pattern: /GROOM_NAME/g, replacement: editCoupleInfo.groomName },
        { pattern: /PARTNER_1/g, replacement: editCoupleInfo.brideName },
        { pattern: /PARTNER_2/g, replacement: editCoupleInfo.groomName },
      ...
      const newScript = {
        id: generatedScripts.length + 1,
        title: `Imported Script - ${editCoupleInfo.brideName.split(' ')[0]} & ${editCoupleInfo.groomName.split(' ')[0]}`,
        type: 'Custom',
      ...
      const confirmMessage = {
        id: aiChatMessages.length + 1,
        role: 'assistant',
        content: `I've successfully imported your ${fileExtension?.toUpperCase()} script and personalized it for ${editCoupleInfo.brideName} & ${editCoupleInfo.groomName}!
